...
{
	static int cnt = 0;
	fprintf(stderr, "service sayhello: %d\n", cnt++);
}
...
{
	static int cnt = 0;
	fprintf(stderr, "service sayhello_to:%s : %d\n", name, cnt++);
	return cnt;
}
...
{
    static int cnt = 0;
    fprintf(stderr, "service saygoodbye: %d\n", cnt++);
}
...
{
    static int cnt = 0;
    fprintf(stderr, "service saygoodbye_to:%s : %d\n", name, cnt++);
    return cnt;
}
...
int main(int argc, char **argv)
{
	...
	/* add service */
	ret = svcmgr_publish(bs, svcmgr, "hello", (void *)hello_service_handler);
    if (ret) {
        fprintf(stderr, "failed to publish hello service\n");
        return -1;
    }
	ret = svcmgr_publish(bs, svcmgr, "goodbye", (void *)goodbye_service_handler);
    if (ret) {
